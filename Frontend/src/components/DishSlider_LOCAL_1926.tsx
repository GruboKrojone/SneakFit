        ...
        <>
          {dishesToRender.map((index) => {
            ...
            return (
              <div
                key={`${dish.id}-${index}`}
                className={`dish-card ${actualIndex === actionCardIndex && lastAction ? `action-${lastAction}` : ""} ${offset === 0 ? "front-card" : ""}`}
                title={offset === 0 ? t("dish_details") : ""}
                style={getCardStyle(index) as React.CSSProperties}
                onClick={offset === 0 ? () => navigate(`/${locale}/dish/${dish.id}`) : undefined}
              ...
              >
                ...
                <div className="dish-info">
                  ...
                  <div className="dish-actions">
                    <button className="action-button btn-pass" title="Pass" onClick={(e) => { e.stopPropagation(); handlePass(); }}>
                      <ThumbDown sx={{ fontSize: 24 }} />
                    </button>
                    <button className="action-button btn-smash" title="Loved" onClick={(e) => { e.stopPropagation(); handleLoved(); }}>
                      <Favorite sx={{ fontSize: 24 }} />
                    </button>
                    <button className="action-button btn-loved" title="Smash" onClick={(e) => { e.stopPropagation(); handleSmash(); }}>
